...
# Replace the input layer with a new layer for 15 channels, and initialize it
new_conv_stem = torch.nn.Conv2d(
    in_channels=15,        # Set to 15 to match your input
    out_channels=32,       # Keep this the same as the original model
    kernel_size=(3, 3),    # 3x3 kernel size
    stride=(2, 2),         # 2x2 stride
    padding=1,             # 1 padding
    bias=False
)

# Initialize the weights for the new input layer (e.g., using Kaiming initialization)
torch.nn.init.kaiming_normal_(new_conv_stem.weight, mode='fan_out', nonlinearity='relu')

# Replace the existing model's first layer with the new layer
model._conv_stem = new_conv_stem.to(device)

print(model._conv_stem.weight.shape)
print("Model's first layer replaced with new layer")


# Adjust the classifier layer to match the number of classes in your problem
...
# # Freeze all layers except for the custom first and classifier layers
